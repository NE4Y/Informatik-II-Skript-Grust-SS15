...
$≤$ [I.V.]$ 2^{(depth l)} - 1 + 1 + 2^{(depth r)} -1$\\
$= 2^{(\textbf{depth l})} + 2^{(\textbf{depth r})} -1$\\
$≤ 2$ x max($ 2^{\textbf{(depth l)}} + 2^{\textbf{(depth r)}}$) -1\\
$= 2 \cdot 2^{\textbf{max((depth l), (depthr)}} -1$\\
$= 2^{(1 + \textbf{max((depth l), (depth r))})}$\\

$\leftarrow$[depth] $2^{\textbf{depth((make-node l x r))}} -1 \surd$\\
