...
Werte die Tests in der Reihenfolge t1,t2,...,tn aus. Sobald $t_i $ \# t ergibt werte Zweig $e_i $ aus. $e_i $ ist das Ergebnis der Fallunterscheidung. Wenn $t_n $ \# f liefert, dann liefere
\begin{center}
...
\begin{center}
	(cond ($<t1> <e1>) (<t2> <e2>) ...$) $\rightsquigarrow    \left\{\begin{array}{cl} <e_{1}>, & \mbox{falls t1' = \#f} \\ (cond (<t_{2}> <e_{2}> ) (...) ), & \mbox{sonst} \end{array}\right. $
\end{center}

Reduziere t1, erhalte t1'.
...
(cond ($<t1> <e1>$) ($<t2> <e2>$) ... ))) entspricht (if $<t1> <e1> (if <t1> <e2> (if ...)$)

Spezialformen 'and' und 'or':
\begin{center}
$(or <t1> <t2> ... <tn>)$ entspricht $(if <t1> \#t (or <t2> ...)$)

(or) $\rightsquigarrow$ \#f
\end{center}
\begin{center}
$(and <t1>...<tn> \rightsquigarrow (if <t1> (and <t2> ... <tn>) \#f$)
